...
- `pages/` 原先约 300 MB，原因是生成器把约 2.1 MB 的共享旧应用完整复制了 148 次。现在 148 个历史 URL 保持不变，但每页只负责跳转到共享壳 `index.html#实验ID`；生成后总计 79,842 字节。
...
1. 历史独立页可由 `python3 gen_pages.py` 从 `experiments_meta.json` 确定性重建。
2. 旧版完整互动仍由根目录 `index.html#实验ID` 承载，148 页回归会逐个从历史 URL 验证跳转后的五阶段和互动区。

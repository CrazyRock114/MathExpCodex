...
当前质量入口会依次运行目录生成、TypeScript、Vitest、Vite 生产构建和 167 项 Playwright 检查：
...
- PR01、PR02、PR03 已具有 React 原生五阶段互动与算法单元测试
- 三个原生实验的全部 15 个阶段在 360px、390px、768px 无横向溢出
- 新目录与三个原生实验的全部阶段没有 axe `critical` 或 `serious` 无障碍问题
...
- PR01 布丰投针、PR02 科拉茨猜想、PR03 勾股数公式的 React 原生五阶段实验
- 3 项已完成来源与数学内容核验，145 项仍明确标记为 `unreviewed`
- 生产包约 86KB gzip；长目录卡片使用 `content-visibility`
...
现有 `index.html` 和独立页面仍包含大量重复代码。三个首批旗舰实验已迁入共享实验壳；其余详情页仍链接旧互动页面，迁移期间保留旧页作为对照。

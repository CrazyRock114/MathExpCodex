...
当前质量入口会依次运行目录生成、TypeScript、Vitest、Vite 生产构建和 187 项 Playwright 检查：
...
- 23 个旗舰实验已具有 React 原生五阶段互动与算法单元测试
- 二十三个原生实验的全部 115 个阶段在 360px、390px、768px 无横向溢出
- 新目录与二十三个原生实验的全部阶段没有 axe `critical` 或 `serious` 无障碍问题
...
- PR01–PR08、SQ01、SQ07、SQ09、GM01、GM03、GM04、GR01、GR02、GR07、PB01–PB03、AL01、AL04、AL07 的 React 原生五阶段实验
- 23 项已完成来源与数学内容核验，125 项仍明确标记为 `unreviewed`
- 实验组件按路由动态加载；入口 JavaScript 约 87.6KB gzip，单个实验块约 2.1–3.3KB gzip
...
现有 `index.html` 和独立页面仍包含大量重复代码。23 个旗舰实验已迁入共享实验壳；其余详情页仍链接旧互动页面，迁移期间保留旧页作为对照。

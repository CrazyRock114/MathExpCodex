...
test('新应用展示全部 148 个实验并支持搜索', async ({ page }) => {
  await page.goto('/dist/index.html', { waitUntil: 'domcontentloaded' });
  await expect(page.locator('.experiment-card')).toHaveCount(148, { timeout: 15_000 });
  const experimentHrefs = await page.locator('.experiment-card h2 a').evaluateAll((links) =>
    links.map((link) => link.getAttribute('href'))
  );
  expect(new Set(experimentHrefs).size).toBe(148);
  expect(experimentHrefs).toContain('./pages/PR01.html');
...
test('独立实验页面提供原生五步实验并保留旧实验对照', async ({ page }) => {
  await page.goto('/dist/pages/PR01.html', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/dist\/pages\/PR01\.html$/);
  await expect(page).toHaveTitle(/投针.*PR01.*MathExpCodex/);
  ...
  await expect(page.getByRole('link', { name: /返回实验目录/ })).toHaveAttribute('href', '../index.html');
});

test('生产入口兼容旧版 #实验ID 深链接', async ({ page }) => {
  await page.goto('/dist/index.html#NT06', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/dist\/pages\/NT06\.html$/);
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Harshad');
  await expect(page.getByRole('link', { name: /打开 NT06 旧版实验/ })).toHaveAttribute(
    'href',
    '../legacy.html#NT06'
  );
});

test('待迁移实验也在独立页面展示且不请求音频', async ({ page }) => {
  const mediaRequests = [];
  page.on('request', (request) => {
    if (/\.(?:mp3|wav|m4a|ogg)(?:\?.*)?$/i.test(request.url())) mediaRequests.push(request.url());
  });
  await page.goto('/dist/pages/NT06.html', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/dist\/pages\/NT06\.html$/);
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Harshad');
  await expect(page.getByText(/尚未完成权威来源核验/)).toBeVisible();
...
test('PR02 明确区分有限计算与未解决猜想', async ({ page }) => {
  await page.goto('/dist/pages/PR02.html', { waitUntil: 'domcontentloaded' });
...
test('PR03 原生实验复现 c≤1000 的 158 组计数', async ({ page }) => {
  await page.goto('/dist/pages/PR03.html', { waitUntil: 'domcontentloaded' });
...
test('PR04 纠正点接触相邻规则的错误上界', async ({ page }) => {
  await page.goto('/dist/pages/PR04.html', { waitUntil: 'domcontentloaded' });
...
test('PR05 精确枚举证明 K6 没有可避开染色', async ({ page }) => {
  await page.goto('/dist/pages/PR05.html', { waitUntil: 'domcontentloaded' });
  await page.getByRole('tab', { name: /枚举/ }).click();
  await expect(page.getByRole('row', { name: /K6 32,768 0 0\.0000%/ })).toBeVisible();
});

test('PR06 准确列出 100 的六个不重复素数分拆', async ({ page }) => {
  await page.goto('/dist/pages/PR06.html', { waitUntil: 'domcontentloaded' });
...
test('PR07 准确给出一万以内 205 对孪生素数', async ({ page }) => {
  await page.goto('/dist/pages/PR07.html', { waitUntil: 'domcontentloaded' });
...
test('PR08 展示 96 边形给出的 π 上下界', async ({ page }) => {
  await page.goto('/dist/pages/PR08.html', { waitUntil: 'domcontentloaded' });
...
test('SQ01 明确斐波那契下标与不完整整除命题', async ({ page }) => {
  await page.goto('/dist/pages/SQ01.html', { waitUntil: 'domcontentloaded' });
...
test('GM01 区分凸多边形扇形切法与一般三角剖分', async ({ page }) => {
  await page.goto('/dist/pages/GM01.html', { waitUntil: 'domcontentloaded' });
...
test('GR01 的完整判据包含连通性', async ({ page }) => {
  await page.goto('/dist/pages/GR01.html', { waitUntil: 'domcontentloaded' });
...
test('PB01 披露 50.73% 的模型假设', async ({ page }) => {
  await page.goto('/dist/pages/PB01.html', { waitUntil: 'domcontentloaded' });
...
test('AL04 修正 64 盘耗时数量级', async ({ page }) => {
  await page.goto('/dist/pages/AL04.html', { waitUntil: 'domcontentloaded' });
...
test('SQ07 修正杨辉三角奇数项个数公式', async ({ page }) => {
  await page.goto('/dist/pages/SQ07.html', { waitUntil: 'domcontentloaded' });
  await page.getByRole('tab', { name: /奇偶/ }).click();
...
test('GM03 只在固定周长下比较等边最大面积', async ({ page }) => {
  await page.goto('/dist/pages/GM03.html', { waitUntil: 'domcontentloaded' });
...
test('GR07 用精确公式替代错误的随机桶碰撞模拟', async ({ page }) => {
  await page.goto('/dist/pages/GR07.html', { waitUntil: 'domcontentloaded' });
...
test('PB02 明确主持人协议并删除伪贝叶斯推导', async ({ page }) => {
  await page.goto('/dist/pages/PB02.html', { waitUntil: 'domcontentloaded' });
...
test('AL07 修正 4 亿规模和 JavaScript 位运算中点', async ({ page }) => {
  await page.goto('/dist/pages/AL07.html', { waitUntil: 'domcontentloaded' });
...
test('SQ09 用拼图和代数证明连续三角数恒等式', async ({ page }) => {
  await page.goto('/dist/pages/SQ09.html', { waitUntil: 'domcontentloaded' });
...
test('GM04 修正球内接正方体边长与最优条件', async ({ page }) => {
  await page.goto('/dist/pages/GM04.html', { waitUntil: 'domcontentloaded' });
...
test('GR02 区分精确最短路线与最近邻启发式', async ({ page }) => {
  await page.goto('/dist/pages/GR02.html', { waitUntil: 'domcontentloaded' });
  await page.getByRole('tab', { name: /规模/ }).click();
...
test('PB03 先披露二项模型条件再讨论近似', async ({ page }) => {
  await page.goto('/dist/pages/PB03.html', { waitUntil: 'domcontentloaded' });
...
test('AL01 用精确有理数找到分数解并修正枚举数量', async ({ page }) => {
  await page.goto('/dist/pages/AL01.html', { waitUntil: 'domcontentloaded' });
...
test('SQ04 修正三对括号枚举并明确计数对象', async ({ page }) => {
  await page.goto('/dist/pages/SQ04.html', { waitUntil: 'domcontentloaded' });
...
test('GM07 用合法环面胞腔分解得到 χ=0', async ({ page }) => {
  await page.goto('/dist/pages/GM07.html', { waitUntil: 'domcontentloaded' });
...
test('GR09 展示度数和与边数的双重计数', async ({ page }) => {
  await page.goto('/dist/pages/GR09.html', { waitUntil: 'domcontentloaded' });
...
test('PB08 区分灵敏度与阳性预测值', async ({ page }) => {
  await page.goto('/dist/pages/PB08.html', { waitUntil: 'domcontentloaded' });
...
test('AL10 从 p² 开始筛并修正十亿以内素数个数', async ({ page }) => {
  await page.goto('/dist/pages/AL10.html', { waitUntil: 'domcontentloaded' });
...
test('NT01 修正完全数范围、定理归属与当前记录', async ({ page }) => {
  await page.goto('/dist/pages/NT01.html', { waitUntil: 'domcontentloaded' });
  await page.getByRole('tab', { name: /搜索/ }).click();
...
test('FR03 区分无限边界、有限区域与不准确现实类比', async ({ page }) => {
  await page.goto('/dist/pages/FR03.html', { waitUntil: 'domcontentloaded' });
...
for (const width of [360, 390, 768]) {
  ...
  test(`三十个原生实验全部阶段在 ${width}px 无横向溢出`, async ({ page }) => {
    ...
    for (const experimentId of NATIVE_EXPERIMENT_IDS) {
      await page.goto(`/dist/pages/${experimentId}.html`, { waitUntil: 'domcontentloaded' });
